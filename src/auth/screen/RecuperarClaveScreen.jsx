    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
           let nuevaClave = {
            ...
            postClave(nuevaClave);
          }
        });
        
      
        const postClave = async (datos) => {
            try{
                
                let response;
                response = await API.post("UserAccount/RecuperarClave", datos);
                ...
                if (response.status == 200 || response.status == 204) {
                    ...
                    accionExitosa({ titulo: 'Codigo Enviado', mensaje: JSON.stringify(response.data)
                        ...
                        if(result.isConfirmed){
                            localStorage.setItem("pag", "RecuperarClave")
                    ...
                    accionFallida({ titulo: 'El email no pudo ser validado', mensaje: 'Ha ocurrido un error inesperado' }); 
                }

            }catch(error){
                setError(error.response.data); 
                accionFallida({ titulo: 'El email no pudo ser validado', mensaje: JSON.stringify(error.response.data) });
            }
        ...
        <div className="d-flex justify-content-center">
            <div className="mt-5 " style={{ width: '500px' }} >
                <Card style={{ "borderColor": "#2596be" }} className={`text-white text-center`}>
                    ...
                    <Card.Body >
                    <Card.Title className={`text-black text-center mt-4 `}>Recuperar Contraseña</Card.Title>
                    <Card.Subtitle className="text-center mt-2 text-muted">*Le enviaremos un código a su correo <br/> para restablecer su contraseña</Card.Subtitle>
                        <div className="p-3">
