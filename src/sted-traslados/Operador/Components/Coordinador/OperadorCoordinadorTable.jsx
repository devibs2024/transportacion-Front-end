...
export const OperadorCordinadorTable = ({ operador, coordinadores, setCoordinadores }) => {
    ...
    const eliminarCoordinador = async (coordinador) => {
        ...
        confirmarAccion({ titulo: 'Eliminar Coordinador', mensaje: 'Estas seguro que deseas eliminar el Coordinador?' }).then(async (result) => {
            if (result.isConfirmed) {
                const response = await API.delete(`OperadorCoordinador`, {
                    headers: {},
                    data: {
                        idOperador: operador.idEmpleado,
                ...
                if (response.status == 200 || response.status == 204) {
                    ...
                    const filteredCoordinadores = coordinadores.filter(x => x.idCoordinador != coordinador.idCoordinador)
    ...
    return (<Table>
        <thead><tr><th>Nombre</th><th className="text-center">Acción</th></tr></thead>
        <tbody>
